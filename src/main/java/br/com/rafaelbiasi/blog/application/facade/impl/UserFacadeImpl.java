...
public class UserFacadeImpl implements UserFacade {
    ...
    @Override
    public UserData save(final UserData postData) {
        requireNonNull(postData, "The User has a null value.");
        return ofNullable(postData.getCode())
                .map(SqidsUtil::decodeId)
                .flatMap(userService::findById)
                .map(user -> update(postData, user))
    ...
    public boolean delete(final String code) {
        requireNonNull(code, "The Code has a null value.");
        val post = userService.findById(SqidsUtil.decodeId(code));
        post.ifPresent(userService::delete);
